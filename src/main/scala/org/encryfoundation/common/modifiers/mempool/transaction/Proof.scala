...
object Proof {
  ...
  implicit val jsonEncoder: Encoder[Proof] = (p: Proof) =>
    Map(
...
object ProofSerializer extends Serializer[Proof] {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[Proof] = Try {
    ...
    val tagOpt: Option[String] =
      if (bytes.lengthCompare(valueLen + 2) != 0)
        Some(new String(bytes.drop(valueLen + 2), Algos.charset))
      else None
    Proof(value, tagOpt)
  }
}
