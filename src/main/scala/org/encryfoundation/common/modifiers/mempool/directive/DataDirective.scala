...
object DataDirective {
  ...
  implicit val jsonDecoder: Decoder[DataDirective] = (c: HCursor) => for {
    contractHash <- c.downField("contractHash").as[String]
    dataEnc      <- c.downField("data").as[String]
  } yield Algos.decode(contractHash).flatMap(ch => Algos.decode(dataEnc).map(data => DataDirective(ch, data)))
