...
object Equihash {
  ...
  def expandArray(inp: Array[Byte], outLen: Int, bitLen: Int, bytePad: Int = 0): Array[Byte] = {
    ...
    inp.indices.foldLeft(0, 0) {
      case ((accBits, j), i) =>
        accValue = accValue
          .shiftLeft(8)
          .and(BigInteger.ONE.shiftLeft(wordSize).subtract(BigInteger.ONE))
        ...
        if (possibleAccBits >= bitLen) {
          ...
          (bytePad until outWidth).foreach(
            x =>
              out.update(
                j + x, {
                  accValue
                    .shiftRight(correctAccBits + (8 * (outWidth - x - 1)))
                    .and(
                      BigInteger
                        .valueOf((1 << bitLen) - 1)
                        .shiftRight(8 * (outWidth - x - 1))
  ...
  def gbpBasic(digest: Blake2bDigest, n: Char, k: Char): Seq[EquihashSolution] = {
    ...
    for (i <- 1 until k) {
      ...
      while (x.nonEmpty) {
        ...
        val j: Int = (1 until xSize)
          .find(j => !hasCollision(x.last._1, x(xSize - 1 - j)._1, i, collisionLength))
          .getOrElse(xSize)
        for {
          l <- 0 until j - 1
          m <- l + 1 until j
    ...
    while (x.nonEmpty) {
      ...
      val j: Int = (1 until XSize)
        .find(
          j =>
            !(hasCollision(x.last._1, x(XSize - 1 - j)._1, k, collisionLength) &&
              hasCollision(x.last._1, x(XSize - 1 - j)._1, k + 1, collisionLength))
        )
        .getOrElse(XSize)
      for {
        l <- 0 until j - 1
  ...
  def validateSolution(n: Char,
                       ...
                       personal: Array[Byte],
                       header: Array[Byte],
                       solutionIndices: IndexedSeq[Int]): Boolean = {
    ...
    if (solutionIndices.toSet.size != solutionIndices.size) false
      ...
      val pairWiseCheck: Boolean = (0 until k).forall(s => {
        val d: Int = 1 << s
        (0 until solutionLen by 2 * d).forall(
          i =>
